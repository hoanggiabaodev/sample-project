        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    return metaUrl ? `<code>${metaUrl}</code>` : '<span class="text-muted">-</span>';
                }
            },
            {
                data: 'status',
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    return `<div class="btn-group" role="group">
                        <button type="button" class="btn btn-sm btn-outline-info" onclick="viewCategory(${data})">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-warning" onclick="editCategory(${data})">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-danger" onclick="deleteCategory(${data})">
...
async function viewCategory(id) {
    try {
        ...
        const result = ApiUtils.handleResponse(response, null, 'Không thể tải thông tin danh mục.');
    ...
    let content = `
        <div class="row">
            <div class="col-md-8">
                <h4>${category.name}</h4>
                <div class="mb-3">
                    <strong>URL Meta:</strong> <code>${category.metaUrl || generateMetaUrl(category.name)}</code>
                </div>
                <div class="mb-3">
                    <strong>Trạng thái:</strong> ${getStatusBadge(category.status)}
                </div>
                <div class="mb-3">
                    <strong>Ngày tạo:</strong> ${formatDate(category.createdAt)}
                </div>
                <div class="mb-3">
                    <strong>Cập nhật lần cuối:</strong> ${formatDate(category.updatedAt)}
                </div>
            </div>
            <div class="col-md-4">
                <div class="card">
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.result === 1) {
                ...
                showToast('error', 'Lỗi', response.error?.message || 'Không thể xóa danh mục');
            }
        },
        error: function () {
...
async function saveCategory() {
    ...
    try {
        ...
        const result = ApiUtils.handleResponse(response, 'Đã lưu danh mục thành công.', 'Không thể lưu danh mục.');
...
async function updateCategory() {
    ...
    try {
        ...
        const result = ApiUtils.handleResponse(response, 'Đã cập nhật danh mục thành công.', 'Không thể cập nhật danh mục.');
...
function buildAddFormData() {
    const formData = {
        ...
        metaUrl: $('#categoryMetaUrl').val() || generateMetaUrl($('#categoryName').val())
    };
...
function buildEditFormData() {
    const formData = {
        ...
        metaUrl: $('#editCategoryMetaUrl').val() || generateMetaUrl($('#editCategoryName').val())
    };
...
$(document).ready(function () {
    $('#categoryName, #editCategoryName').on('input', function () {
        const name = $(this).val();
        const metaUrlField = $(this).attr('id') === 'categoryName' ? '#categoryMetaUrl' : '#editCategoryMetaUrl';
