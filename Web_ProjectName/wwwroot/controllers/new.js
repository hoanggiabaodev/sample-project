    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response && response.result === 1 && response.data && Array.isArray(response.data)) {
                ...
                var colorClasses = ['color1', 'color2', 'color3'];
                ...
                limitedNews.forEach(function(news, index) {
                    ...
                    var newsHtml = '<div class="trand-right-single d-flex mb-3 p-2" style="border-radius: 8px; background-color: #f8f9fa;">' +
                                   ...
                                   '<img src="' + imageUrl + '" alt="' + news.name + '" style="width: 70px; height: 50px; object-fit: cover; border-radius: 6px;">' +
                                   '</div>' +
                                   '<div class="trand-right-cap flex-grow-1" style="padding-left: 8px;">' +
                                   '<span class="' + colorClass + '" style="font-size: 11px; font-weight: 500; text-transform: uppercase; letter-spacing: 0.5px;">' + (news.newsCategoryObj?.name || 'Tin tức') + '</span>' +
                                   '<h6 style="font-size: 13px; font-weight: 600; line-height: 1.3; margin: 2px 0 0 0;"><a href="/tin-tuc/' + news.metaUrl + '" style="text-decoration: none; color: #333; transition: color 0.2s;" onmouseover="this.style.color=\'#0056b3\'" onmouseout="this.style.color=\'#333\'">' + news.name + '</a></h6>' +
                                   '</div>' +
                                   '</div>';
                ...
                console.log('Không có tin tức nổi bật nào hoặc cấu trúc phản hồi không hợp lệ');
                $('#featured-news-container').html('<div class="text-center"><p class="text-muted">Không có tin tức nổi bật nào</p></div>');
            }
        },
        error: function (error) {
            console.log('Error loading featured news:', error);
            $('#featured-news-container').html('<div class="text-center"><p class="text-danger">Lỗi tải tin tức nổi bật</p></div>');
        }
    });
}
